...
async def start_pipeline(
    ...
    Validates the video source, checks for an existing pipeline lock,
    then enqueues the request on the Redis stream.

    Args:
        request: Pipeline start request with video source and config

    Returns:
        Pipeline start response with request_id and status
    ...
    # Prepare configuration dictionary (exclude video source and download fields)
    config = request.dict(exclude={"video_id", "video_url", "force_download"})

    # Include download-specific fields in config so the worker has them
    if video_url:
    ...
    # Enqueue on the pipeline stream
